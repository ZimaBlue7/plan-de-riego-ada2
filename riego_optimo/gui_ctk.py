...
from io_handler import leer_finca_desde_archivo, escribir_solucion_a_archivo
...
class RiegoOptimoApp(ctk.CTk):
    def __init__(self):
        ...
        self.last_best_sol: Optional[Solucion] = None
        ...
        self.logo_label = ctk.CTkLabel(
            self.sidebar, text="Riego Óptimo", font=ctk.CTkFont(size=24, weight="bold")
        )
        self.logo_label.pack(pady=(20, 10))

        # Sección de Algoritmos
        self.algo_label = ctk.CTkLabel(
            self.sidebar, text="Algoritmos:", font=ctk.CTkFont(weight="bold")
        )
        self.algo_label.pack(pady=(20, 5), anchor="w", padx=20)

        self.var_fb = tk.BooleanVar(value=True)
        self.check_fb = ctk.CTkCheckBox(
            self.sidebar, text="Fuerza Bruta (FB)", variable=self.var_fb
        )
        self.check_fb.pack(pady=5, anchor="w", padx=30)

        self.var_v = tk.BooleanVar(value=True)
        self.check_v = ctk.CTkCheckBox(
            self.sidebar, text="Voraz (V)", variable=self.var_v
        )
        self.check_v.pack(pady=5, anchor="w", padx=30)

        self.var_pd = tk.BooleanVar(value=True)
        self.check_pd = ctk.CTkCheckBox(
            self.sidebar, text="Prog. Dinámica (PD)", variable=self.var_pd
        )
        ...
        self.rep_label = ctk.CTkLabel(
            self.sidebar,
            text="Repeticiones para promedio:",
        ...
        self.btn_file = ctk.CTkButton(
            self.top_bar, text="Cargar desde Archivo .txt", command=self.cargar_archivo
        )
        ...
        self.btn_clear = ctk.CTkButton(
            self.top_bar,
            text="Limpiar Editor",
            command=lambda: self.data_editor.delete("0.0", "end"),
        ...
        self.btn_save = ctk.CTkButton(
            self.top_bar,
            text="Guardar mejor solución",
            command=self.guardar_mejor_solucion,
        )
    ...
    def cargar_archivo(self):
        ...
        if filename:
            try:
                ...
                messagebox.showinfo(
                    "Éxito", "Archivo cargado correctamente en el editor."
                )
    ...
    def parse_finca_from_editor(self) -> Optional[Finca]:
        ...
        try:
            ...
            if len(lines) < n + 1:
                raise ValueError(
                    f"Se esperaban {n} tablones, pero solo hay {len(lines)-1} líneas de datos."
                )
            ...
            for i in range(1, n + 1):
                ...
                if len(partes) != 4:
                    raise ValueError(
                        f"Línea {i+1}: Se esperaban 4 valores (ts,tr,p,rp), se encontraron {len(partes)}"
                    )
            ...
            messagebox.showerror(
                "Error de Formato", f"Error al procesar los datos:\n{e}"
            )
            return None
    ...
    def ejecutar_procesamiento(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", "El número de repeticiones debe ser un entero positivo."
            )
            return
        ...
        def worker():
            try:
                ...
                self.after(
                    0, lambda: messagebox.showerror("Error en ejecución", str(e))
                )
            finally:
                self.after(
                    0,
                    lambda: self.btn_procesar.configure(
                        state="normal", text="Procesar Finca"
                    ),
                )
    ...
    def medir_todo(self, finca: Finca, reps: int):
        algos_to_run = []
        if self.var_fb.get():
            algos_to_run.append(("Fuerza Bruta", roFB))
        if self.var_v.get():
            algos_to_run.append(("Voraz", roV))
        if self.var_pd.get():
            algos_to_run.append(("Prog. Dinámica", roPD))
        ...
        if finca.n <= 10:
            try:
                if any(nombre == "Fuerza Bruta" for nombre, _ in algos_to_run):
                    pass
                else:
                    solucion_optima = roFB(finca)
            except Exception:
        ...
        for nombre, fn in algos_to_run:
            ...
            if error:
                results_data.append({"algoritmo": nombre, "error": error})
            else:
                avg_time = sum(tiempos) / len(tiempos)
                es_optima = None
                if nombre == "Fuerza Bruta":
                    es_optima = True
                    solucion_optima = ultima_sol
                elif solucion_optima is not None:
                    es_optima = (
                        abs(ultima_sol.costo_total - solucion_optima.costo_total) < 1e-9
                    )
                results_data.append(
                    {
                        "algoritmo": nombre,
    ...
    def mostrar_resultados(self, resultados):
        ...
        # Tabla comparativa
        header = f"{'Algoritmo':<20} | {'Costo':<12} | {'Promedio (ms)':<15} | {'Optimo':<7}\n"
        separator = "-" * 70 + "\n"
        ...
        for res in resultados:
            if "error" in res:
                ...
                estado_opt = (
                    "N/A"
                    if res.get("es_optima") is None
                    else ("Si" if res.get("es_optima") else "No")
                )
                resumen += f"{res['algoritmo']:<20} | {res['costo']:<12.2f} | {res['tiempo_avg']:<15.4f} | {estado_opt:<7}\n"
        ...
        if valid_results:
            ...
            self.last_best_sol = mejor.get("solucion")
    ...
    def guardar_mejor_solucion(self) -> None:
        if self.last_best_sol is None:
            messagebox.showwarning("Aviso", "No hay una solución para guardar.")
            return
        ...
        try:
            escribir_solucion_a_archivo(self.last_best_sol, filename)
